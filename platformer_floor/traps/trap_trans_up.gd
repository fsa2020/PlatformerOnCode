...
func onTargetEnter(target):
	var moveY = label.global_position.y - target.global_position.y - 50
	...
	var moveUp = Actions.MoveTo.new(target,upPos,yTime)
...
func addExtraBody():
	...
	capShape.shape = rect
	capShape.position = Vector2(label.size.x/2, 0)
